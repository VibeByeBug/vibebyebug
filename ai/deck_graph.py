"""발표자료 슬라이드 지도 - 슬라이드 사이의 주장과 근거를 잇는다 (GraphRAG 식).
...
지도 화면을 열 때 모델이 자료 전체를 한 번 읽고 세 가지를 만든다(전에는 준비 단계에서 만들었다).
  역할   슬라이드마다 문제 / 방법 / 결과 / 근거 / 한계 / 제안 / 소개
  연결   "9번 주장은 10번이 뒷받침한다" 같은 슬라이드 사이 관계
  줄거리 발표 전체 흐름 5~6줄

지금은 화면(지도의 "슬라이드 지도" 탭)에서만 쓴다. 답변 경로에서는 뺐다.
검색을 통합 지식 조각으로 바꾼 뒤 다시 재보니 지도가 있으나 없으나 근거 슬라이드가
12문항 모두 같았다(2026-09-20, ai/README.md). 검색이 지도의 역할을 흡수했다.

조각 사이의 관계는 "지식 지도"(knowledge_graph.py)다. 둘은 점의 단위가 다르다.
"""
...
def build(rows: list[dict], notes: list[dict] | None = None) -> dict:
    """슬라이드 지도를 만든다. 실패하면 빈 지도를 돌려준다(없어도 검색은 지금처럼 동작한다).
